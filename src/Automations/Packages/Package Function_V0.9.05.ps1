...
function Create-PCXPackage {
    ...
    try {
        ...
        # INSTALL
        Invoke-PCXWithRetry {
            New-CMProgram -PackageName $meta.Name -StandardProgramName "$($meta.Name) [INSTALL]" `
                -CommandLine (Get-PCXInstallCommand $installer) `
                -RunMode RunWithAdministrativeRights -ProgramRunType WhetherOrNotUserIsLoggedOn
        }

        # UNINSTALL
        Invoke-PCXWithRetry {
            New-CMProgram -PackageName $meta.Name -StandardProgramName "$($meta.Name) [UNINSTALL]" `
                -CommandLine (Get-PCXUninstallCommand $installer) `
                -RunMode RunWithAdministrativeRights -ProgramRunType WhetherOrNotUserIsLoggedOn
        }

        Write-PCXLog "Install + Uninstall created"

        # BAT programs
        foreach ($p in Get-PCXBatchPrograms $files $meta.Name) {
            Invoke-PCXWithRetry {
                New-CMProgram -PackageName $meta.Name -StandardProgramName $p.Name -CommandLine $p.Cmd `
                    -RunMode RunWithAdministrativeRights -ProgramRunType WhetherOrNotUserIsLoggedOn
            }

            Write-PCXLog "BAT added: $($p.Name)"
        }
