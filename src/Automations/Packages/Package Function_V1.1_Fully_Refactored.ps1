...
if (-not $script:Config) {
    $script:Config = @{
        ...
        RetryCount               = 3
        RetryDelaySeconds        = 5
        DefaultDPGroup           = 'All Mangalore Dps'
        DefaultLimitingCollection= 'ALL Systems'
        ProgramDurationMinutes   = 20
        DiskSpaceRequirement     = 5
        DiskSpaceUnit            = 'GB'
    }
}

# Cache for supported platforms
$script:Windows11Platforms = $script:Windows11Platforms
...
function Get-PCXCommandLine {
    ...
    Set-Location c:
    # FIX: no -File used
    Get-ChildItem -LiteralPath $Path | Where-Object { -not $_.PSIsContainer } | ForEach-Object {
        $map[$_.Name.ToLower()] = $_
    }
    # Removed in V1.1: SCCM connection is not required here.
    switch ($Type) {
...
function New-SCCMCollections {
    ...
    Write-PCXLog "Collections created" -ForegroundColor Green
}
...
function Start-SCCMContentDistribution {
    ...
    Write-PCXLog "Content distributed to DP Group: $DistributionPointGroupName" -ForegroundColor Green
}
...
function New-SCCMDeployments {
    ...
    Write-PCXLog "Available Program: $($Programs.Available)"
    Write-PCXLog "Install Program: $($Programs.Install)"
    Write-PCXLog "Uninstall Program: $($Programs.Uninstall)"

    Write-PCXLog "Available Collection: $($Collections.Available)"
    Write-PCXLog "Install Collection: $($Collections.Install)"
    Write-PCXLog "Uninstall Collection: $($Collections.Uninstall)"
    ...
    Write-PCXLog "Deployments created" -ForegroundColor Green
}
...
function Move-SCCMCollectionsToFolder {
    ...
    foreach ($collection in $collectionList) {
        ...
        Write-PCXLog "Moved Collection: $collection" -ForegroundColor Green
    }
}
...
function Move-SCCMPackageToFolder {
    ...
    $packageObject = Get-CMPackage -Name $meta.PackageName

    Move-CMObject `
        -FolderPath $folderPath `
        -InputObject $packageObject

    Write-PCXLog "Moved Package: $($meta.PackageName)" -ForegroundColor Green
}
...
function Set-SCCMCollectionRules {
    ...
    Write-PCXLog "Collection membership rules configured" -ForegroundColor Green
}
...
function Create-PCXPackage {
    ...
    try {
        ...
        $packageInfo = Get-PCXPackageMetadata -Path $Path
